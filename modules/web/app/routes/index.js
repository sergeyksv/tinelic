...
define(['require', 'tinybone/backadapter', 'safe', 'lodash', 'feed/mainres', 'moment'], (require, api, safe, _, feed, moment) => (req, res, cb) => {
	...
	safe.parallel({
		...
		var _fv = r.teams[1];
		r.teams = r.teams[0];
		_.forEach(r.teams, (team) => {
			var projects = {};
			_.forEach(r.data, (proj) => projects[proj._id] = proj);
			_.forEach(team.projects, (proj) => proj._t_proj = projects[proj._idp]);

			var tmetrics = {};
			_.forEach(team.projects, (proj) => _.assign(tmetrics, _.pick(proj._t_proj, 'apdex', 'server', 'client', 'errAck', 'ajax'),
				(oval, sval, key) => {
					var memo = {};
					var rpm = 1, k;
